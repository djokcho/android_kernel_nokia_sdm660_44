...
if [ -f split_img/*-sigtype ]; then
  ...
  echo " ";
  case $sigtype in
    AVBv1) java -jar "$bin/boot_signer.jar" /$avbtype unsigned-new.img "$avbkey.pk8" "$avbkey.x509."* latestboot.img 2>/dev/null;;
    AVBv2) echo "AVBv2 detected, no need to sign.";;
    BLOB)
      awk 'BEGIN { printf "-SIGNED-BY-SIGNBLOB-\00\00\00\00\00\00\00\00" }' > latestboot.img;
      "$bin/$arch/blobpack" blob.tmp $blobtype unsigned-new.img >/dev/null;
      cat blob.tmp >> latestboot.img;
      rm -f blob.tmp;
    ;;
    CHROMEOS) "$bin/$arch/futility" vbutil_kernel --pack latestboot.img --keyblock "$bin/chromeos/kernel.keyblock" --signprivate "$bin/chromeos/kernel_data_key.vbprivk" --version 1 --vmlinuz unsigned-new.img --bootloader "$bin/chromeos/empty" --config "$bin/chromeos/empty" --arch arm --flags 0x1;;
    DHTB)
      "$bin/$arch/dhtbsign" -i unsigned-new.img -o latestboot.img >/dev/null;
      rm -f split_img/*-tailtype 2>/dev/null;
    ;;
    NOOK*) cat split_img/*-master_boot.key unsigned-new.img > latestboot.img;;
  esac;
  if [ ! $? -eq "0" ]; then
    abort;
    exit 1;
...
if [ -f split_img/*-lokitype ]; then
  ...
  echo " ";
  mv -f latestboot.img unlokied-new.img;
  if [ -f aboot.img ]; then
    "$bin/$arch/loki_tool" patch $lokitype aboot.img unlokied-new.img latestboot.img >/dev/null;
    if [ ! $? -eq "0" ]; then
  ...
  echo " ";
  cp -f latestboot.img unamonet-new.img;
  cp -f split_img/*-microloader.bin microloader.tmp;
  dd bs=1024 count=1 conv=notrunc if=unamonet-new.img of=head.tmp 2>/dev/null;
  dd bs=1024 seek=1 conv=notrunc if=head.tmp of=latestboot.img 2>/dev/null;
  dd conv=notrunc if=microloader.tmp of=latestboot.img 2>/dev/null;
...
if [ -f split_img/*-tailtype ]; then
  ...
  echo " ";
  case $tailtype in
    Bump) awk 'BEGIN { printf "\x41\xA9\xE4\x67\x74\x4D\x1D\x1B\xA4\x29\xF2\xEC\xEA\x65\x52\x79" }' >> latestboot.img;;
    SEAndroid) printf 'SEANDROIDENFORCE' >> latestboot.img;;
  esac;
fi;
...
if [ "$origsize" -a -f split_img/*-origsize ]; then
  ...
  echo " ";
  cp -f latestboot.img unpadded-new.img;
  truncate -s $filesize latestboot.img;
fi;
